#!/usr/bin/env node
// p2p-observe CLI. One command, dry-run by default.
//
//   npx p2p-observe init            # detect project + PREVIEW the plan (writes nothing)
//   npx p2p-observe init --write    # apply the plan (never overwrites existing files)
//   npx p2p-observe gui [--port N]  # launch the inspector GUI (http://localhost:9420)
//
...
function runInit() {
  ...
  console.log(`p2p-observe init — detected runtime: ${runtime}`);
...
if (cmd === 'init') {
  ...
  console.log('Usage: p2p-observe <init [--write] | gui [--port N] [--demo]>');
