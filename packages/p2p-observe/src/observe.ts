...
import { BatchFlusher } from '../../p2p-probe/core/flush.ts';
import { createHyperswarmExporter } from '../../p2p-probe/exporters/hyperswarm.ts';
import type { WritableStreamLike, HyperswarmExporter } from '../../p2p-probe/exporters/hyperswarm.ts';
import { Redactor } from '../../p2p-probe/core/redactor.ts';
import type { EventSink, L2Event } from '../../p2p-probe/src/sink.ts';
...
function persistentId(runtime: string): string {
  try {
    ...
    if (runtime === 'browser' && ls) {
      const KEY = 'p2p-observe:deviceId';
