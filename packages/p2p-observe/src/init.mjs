// Pure planner for `p2p-observe init`. Given a host project's package.json (and a
...
const WORKLET_SCAFFOLD = `// p2p-observe.worklet.mjs — runs INSIDE a react-native-bare-kit Worklet (Bare runtime).
...
//   import source from './p2p-observe.worklet.mjs';   // bundled by bare-kit
//   const worklet = new Worklet();
//   worklet.start('/p2p-observe.worklet.mjs', source);
//   // pipe your transport's events to worklet.IPC`;

const DESKTOP_SNIPPET = `// Desktop (Electron main / Node) — the core runs in-process:
//   import { observe, instrumentHyperswarmStream } from '@p2p/observe';
...
export function planInit({ pkg = {}, hasFile = () => false } = {}) {
  ...
  if (runtime === 'react-native') {
    if (!hasFile('p2p-observe.worklet.mjs')) {
      actions.push({
        path: 'p2p-observe.worklet.mjs',
        kind: 'create',
