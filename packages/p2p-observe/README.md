...
npx p2p-observe init        # detect the project, preview config (dry run)
npx p2p-observe init --write # apply (never overwrites existing files)
```
...
```
Then run the GUI: `npx p2p-observe gui` → open http://localhost:9420.
