...
- `packages/p2p-probe/src/sink.ts` — `L2Event` interface, `L2EventType` union.
- `packages/p2p-protocol/src/envelope.ts` — `P2PEnvelope` frozen wire fields.
...
would duplicate `@p2p/protocol` and drift. So the app hands the native side the
