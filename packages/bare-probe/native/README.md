...
- `packages/bare-probe/src/sink.ts` — `L2Event` interface, `L2EventType` union.
- `packages/bare-protocol/src/envelope.ts` — `P2PEnvelope` frozen wire fields.
...
would duplicate `@holepunchto/bare-protocol` and drift. So the app hands the native side the
